...
%others equal to zero bcz we won't be able to frame them
...
[trainFsst, valFsst, testFsst] = Function_FeaturesExtraction(grps{1}, 1000);
...
[net, info] = Function_training(grps{1}.train(:,2), trainFsst, valFsst, grps{1}.validation(:,2));

%predTrain is predicted value of labels on the training data
%trainScores is a n*k matrix
[trainPred, scoresTrain] = classify(net,trainFsst,'MiniBatchSize',50);

%PredTest is the predicted value for test data
%scores for some sample data
[testPred, scoresTest] = classify(net,testFsst,'MiniBatchSize',50);
